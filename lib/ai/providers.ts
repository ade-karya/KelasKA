...
export const PROVIDERS: Record<ProviderId, ProviderConfig> = {
  ...
  opencode: {
    ...
    models: [
      ...
      // OpenCode Go (paid tier, verified 2026-09-24). Same local-binary
      // transport and the same text-only limitation as the free entries
      // above; the `opencode-go/` prefix is what routes the run through the
      // CLI's stored Go subscription (see toOpencodeModelRef). Ids are kept
      // in ref form (`opencode-go/<model>`) so the tier survives the
      // `opencode:opencode-go/<model>` model string.
      {
      ...
      {
        id: 'opencode-go/mimo-v2.6-flash',
        name: 'MiMo-V2.6-Flash',
        contextWindow: 1048576,
        outputWindow: 131072,
        capabilities: { streaming: true, tools: false, vision: false },
