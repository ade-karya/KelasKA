 ...
 */
export function formatImageDescription(img: PdfImage, language: string = 'id-ID'): string {
  let dimInfo = '';
 ...
 */
export function formatImagePlaceholder(img: PdfImage, language: string = 'id-ID'): string {
  let dimInfo = '';
  if (img.width && img.height) {
    const ratio = (img.width / img.height).toFixed(2);
    dimInfo = ` | size: ${img.width}×${img.height} (aspect ratio ${ratio})`;
