...
React component for rendering PPTist-style `Slide` JSON. Extracted from [KelasKA]().
...
```bash
pnpm add @kelaska/renderer
# or
npm install @kelaska/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@kelaska/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@kelaska/renderer';
...
### Granular components — `@kelaska/renderer/elements`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@kelaska/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
[`@kelaska/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @kelaska/renderer` end-to-end.
