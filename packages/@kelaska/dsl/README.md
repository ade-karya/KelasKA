...
The **contract keystone** of the KelasKA SDK family. `@kelaska/dsl` is *pure spec* — the
...
package can depend on `@kelaska/dsl` without pulling in junk.
...
`@kelaska/dsl` is the only package everything else depends on, and it depends on
nothing.
...
```ts
import type { Slide, PPTElement } from '@kelaska/dsl';
import { isTextElement, DSL_VERSION } from '@kelaska/dsl';
```

## Status

Both consumers are now wired to `@kelaska/dsl` and no longer vendor their own copy
of the slide types:

- **`@kelaska/importer`**: imports all slide types from `@kelaska/dsl`; vendored
  `kelaska/types/slides.ts` deleted. The importer emits complete DSL `Slide`
  objects directly (the old partial "draft slide" + post-fill step is gone).
- **`@kelaska/renderer`**: imports all slide types from `@kelaska/dsl`; vendored
  `types/slides.ts` deleted. `@kelaska/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@kelaska/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@kelaska/importer` to import types from `@kelaska/dsl` (vendored copy deleted).
- [x] Wire `@kelaska/renderer` to import types from `@kelaska/dsl` (vendored copy deleted).
...
- [ ] Reserve `@kelaska/exporter` as the 4th family member.
...
```bash
pnpm --filter @kelaska/dsl build      # -> dist/ (index.js, index.d.ts, …)
pnpm --filter @kelaska/dsl typecheck
```

## License

AGPL-3.0, matching the rest of the family (`@kelaska/dsl`, `@kelaska/importer`,
`@kelaska/renderer`) and the KelasKA root, so the license policy is uniform
across the SDK.
