...
KelasKA supports three deployment styles. All three speak the same KelasKA adapter; you only toggle the backend in Settings.
...
## 2. Point KelasKA at it

Two ways. Pick one.

### A. Per-user (Settings UI, no server change)

Open **Settings → Text-to-Speech → VoxCPM2**, pick the backend, and paste your Base URL. The Request URL preview confirms KelasKA will hit the right endpoint.
...
KelasKA generates a voice prompt from each agent's persona at synthesis time. No setup required. This is what you get if you don't change anything.
