...
KelasKA reads configuration from environment variables at server start. All of them are optional — enable the providers and features you need. For a full list see [`.env.example`](/blob/main/.env.example) in the repo. For built-in model IDs, see [Supported Models](./supported-models.mdx).
...
No API key is needed. Set the base URL so KelasKA bypasses its SSRF check:
...
Without a server-side parser, KelasKA falls back to `unpdf`.
