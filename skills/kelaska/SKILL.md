...
- Do not assume the OpenClaw agent's own model or API key will be reused by KelasKA.
- KelasKA classroom generation uses KelasKA server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only KelasKA server-side config files may control provider selection and defaults.
...
If no `accessCode` in config, ask the user how they want to use KelasKA:

1. **Use hosted KelasKA** (recommended for quick start) — Requires an access code from kelas-ka.riau.ai. No local setup needed.
...
Use this when the user has not installed KelasKA yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify KelasKA

After the user has chosen a startup mode and configured keys, start KelasKA using the chosen method, then verify the service with `GET {url}/api/health`.
