 ...
 *
 * The app loads the parser at runtime from `/vendor/kelaska-importer/index.js` — a
 * gitignored build artifact that `scripts/sync-kelaska-importer.mjs` copies into
...
const entry = path.join(root, 'public/vendor/kelaska-importer/index.js');
...
try {
  ...
  console.error('[assert-vendor] It is loaded at runtime via /vendor/kelaska-importer/index.js');
  console.error('[assert-vendor] and is produced by the postinstall sync step. To fix:');
  console.error(
    '[assert-vendor]   pnpm --filter @kelaska/importer build && pnpm run sync:kelaska-importer',
  );
